...
# 原图片、标签文件、裁剪图片路径
img_path = r'E:\PythonProject\Data\cycgan\images'
xml_path = r'E:\PythonProject\Data\cycgan\xml'
obj_img_path = r'E:\PythonProject\Data\3'
    ...
    if img_file[-4:] in ['.png', '.jpg']:  # 判断文件是否为图片格式
        ...
        if os.path.exists(xml_name):  # 判断与图片同名的标签是否存在，因为图片不一定每张都打标
            ...
            for obj in root.iter('object'):  # 遍历所有目标框
                ...
                print(Numpic)
                #my_file = Path(obj_img_path + '\\' + img_name)
                my_file = Path(obj_img_path)
                # 判断当前name对应的类别有无文件夹
                n=n+1
                '''if 1 - my_file.is_dir():  # 无则新建
                    os.mkdir(obj_img_path + '\\' + str(img_name))'''

                '''cv2.imwrite(obj_img_path + '\\' + img_name + '\\' + '%04d' % (Numpic[name])+'_'+str(n) + '.jpg',
